<!--
  ...
  ITS FONTS COME FROM A CDN, and deliberately. Sveltia injects @font-face
  rules pointing at cdn.jsdelivr.net (Fontsource) for Material Symbols — which
  IS its entire icon set — plus Source Sans 3 and Noto Mono. The CSP below
  allows that one host in font-src and nothing else new; drop it and John
  opens the editor to find buttons with no symbols on them.

  Vendoring those faces is not worth it. The privacy that matters here is
  JOHN'S CLIENTS', and they never see this page: the public site serves every
  font from its own origin, sets no cookies and calls nothing third-party,
  which is why it needs no consent banner. John is one person who knows he is
  signing into GitHub to edit his own website; sparing him one CDN request
  would cost 3.9 MB of icon font in the build and a font pipeline to maintain,
  and buy nothing his visitors can tell the difference about.
-->
<script lang="ts">
